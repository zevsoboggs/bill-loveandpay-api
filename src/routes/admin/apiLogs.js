...
// GET /api/admin/api-logs — all relay API requests (read-only)
router.get('/', async (req, res) => {
  try {
    const { skip, take, orderBy, where } = parseList(req.query, {
      allowedFilters: ['clientId', 'status', 'method', 'sandbox'],
      numericFields: ['status'],
    });
    const [rows, total] = await Promise.all([
      prisma.apiRequestLog.findMany({ skip, take, orderBy, where, include: { client: { select: { name: true } } } }),
